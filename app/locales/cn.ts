...
13. **\`自定义函数调用\`**  
    1. 如需使用函数调用请切换至OpenAI 系列模型,
    2. 根据[自定义函数开发教程](https://forum.xr21.me/forum-post/362.html?self/) 编写自定义groovy脚本
    3. 选中groovy文件右键菜单 **\`加载GPT回调函数\`** 将脚本文件编译为Java并将其中定义的方法注册为GPT回调函数。
    4. 通过该功能为X-Copilot扩展任意自定义能力。
    5. **\`所有的脚本将在你的本地主机执行\`**
    6. 内置函数 **\`news\`** |获取过去第N天的新闻
    7. 内置函数 **\`executeCommand\`** |使用自然语言在你的本地系统执行ps脚本 例如 ： 使用edge打开baidu /打开win11系统设置/
    8. 内置函数 **\`githubSearch\`** | 搜索github仓库
