...
17. 如有 **\`界面卡顿\`** **\`Help\`** > **\`Edit Custom VM Options\`** > 新增一行 **\`-Dfile.encoding=UTF-8\`** > **\`重启\`**
18. 如有 **\`界面卡顿\`** 请新建会话即可
19. 如果您的模型列表与公告中的列表不一致 请执行 **\`设置\`**>**\`重置所有设置\`**>**\`立即重置\`**
20. 如有无法解决的异常请执行 **\`文件\`**>**\`使缓存失效\`**>**\`删除嵌入式浏览器引擎缓存和 Cookie\`**>**\`重启\`** 这将清空所有设置/缓存/聊天记录
21. **\`免费计划\`** 注册登录即加入免费计划 可使用全部免费模型
22. **\`付费计划\`** 只需 **\`￥9.9/周\`**、**\`￥39.9/月\`** 加入付费计划 即可使用全部付费模型
23. **\`速率限制\`** 无论免费还是付费模型 **\`每小时/100/IP\`**
24. **\`价格/速率限制\`** 如有变更以社区公告为准
